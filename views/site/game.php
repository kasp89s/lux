<?php
$header = 'https://cdn.cloudflare.steamstatic.com/steam/apps/::id::/header.jpg?t=';
?>
<div class="container">
    <div class="row mb-2">
        <div class="col-md-12">
            <div class="row no-gutters border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
                <div class="col p-4 d-flex flex-column position-static">
                    <h3 class="mb-0"><?php echo $game->title?></h3>
                </div>
                <div class="col-auto d-none d-lg-block">
                    <img src="<?php echo str_replace('::id::', $game->id, $header)?>" />
                </div>
            </div>
        </div>

    </div>
</div>
<main role="main" class="container">
    <div class="row">
        <div class="col-md-8 blog-main">
            <?php foreach ($news as $new):?>
            <?php
                $_pdo = new \PDO("sqlite:games.db");
                $STH = $_pdo->prepare("SELECT gid FROM favorites WHERE gid = :gid");
                $STH->execute(['gid' => $new['gid']]);
                $STH->setFetchMode(\PDO::FETCH_OBJ);
                $favorite = $STH->fetch();
            ?>
            <div class="blog-post">
                <h2 class="blog-post-title">
                    <?= $new['title']?>
                </h2>
                <p class="blog-post-meta">
                    <?= \Yii::$app->formatter->asDate($new['date']) ?> by
                    <a href="javascript:void(0)"><?php echo $new['feedname']?></a>
                </p>
                <p>
                    Источник: <a href="<?= $new['url']?>"><?php echo $new['url']?></a>
                </p>
                <?php if (!Yii::$app->user->isGuest):?>
                <p>
                    Избранное
                    <?php if (empty($favorite->gid)):?>
                    <span class="add-favorite" style="cursor: pointer;" data-data="<?= urlencode(json_encode($new))?>">
                        <svg width="1.5em" height="1.5em" viewBox="0 0 16 16" class="bi bi-star" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" d="M2.866 14.85c-.078.444.36.791.746.593l4.39-2.256 4.389 2.256c.386.198.824-.149.746-.592l-.83-4.73 3.523-3.356c.329-.314.158-.888-.283-.95l-4.898-.696L8.465.792a.513.513 0 0 0-.927 0L5.354 5.12l-4.898.696c-.441.062-.612.636-.283.95l3.523 3.356-.83 4.73zm4.905-2.767l-3.686 1.894.694-3.957a.565.565 0 0 0-.163-.505L1.71 6.745l4.052-.576a.525.525 0 0 0 .393-.288l1.847-3.658 1.846 3.658a.525.525 0 0 0 .393.288l4.052.575-2.906 2.77a.564.564 0 0 0-.163.506l.694 3.957-3.686-1.894a.503.503 0 0 0-.461 0z"/>
                        </svg>
                    </span>
                    <?php else: ?>
                        <svg width="1.5em" height="1.5em" viewBox="0 0 16 16" class="bi bi-star-fill" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                            <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.283.95l-3.523 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
                        </svg>
                    <?php endif;?>
                </p>
                <?php endif;?>
                <?php echo $new['contents']?>
            </div><!-- /.blog-post -->
            <?php endforeach;?>
        </div><!-- /.blog-main -->

        <aside class="col-md-4 blog-sidebar">
        </aside><!-- /.blog-sidebar -->

    </div><!-- /.row -->

</main>